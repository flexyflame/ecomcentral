...
<?php  
    ...
    $parameter = "";
    $parameter = "text_id_student=00004"; // . $id_student;

    //http://api.derrickbasoah.com/api/student/student_list.php?api_user=admin&text_id_student=00004
    $rest_daten = new ClassRESTDaten($SESSION);
    $xml = REST_POST("/api/student/student_list.php",  $parameter, $rest_daten, true);
    //print_r($xml);
    $xml_obj = simplexml_load_string($xml);
    if ($xml_obj == NULL) {
        //print_r("Error loading...!!!");
        $err=True;
        $message = "Error: Server is stopped! Please inform your supervisor.";
        echo "<script>alert('$message');</script>";
    } else {
        foreach ($xml_obj->student_list->student as $obj_eintrag) {
          print_r($obj_eintrag);
          echo "</br>";
        } 
    }
...
<div class="main-page" style="min-height: 660px; border: 1px solid #4c9cb4; border-radius: 10px; margin-bottom: 10px; max-width: 1260px;">
    ...
    <a class="button primary" href="/output_files.php?page_aktion=excel" onclick="SpinnerBlock();">Export Excel<i class="fa fa-file-excel" aria-hidden="true"></i></a>

    <a class="button primary" href="/gen_pdf.php?page_aktion=pdf" target="_blank" onclick="SpinnerBlock();">Export PDF<i class="fa fa-file-excel" aria-hidden="true"></i></a>
